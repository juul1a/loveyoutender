...
public class Sunset : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        myCoolGradients = new Gradient[fade.Length+1]; //# of fades + skybox
        colorKeys = new GradientColorKey[fade.Length+1][];
        alphaKeys = new GradientAlphaKey[fade.Length+1][];
        ...
        for(int i = 1; i < fade.Length+1; i++){
            myCoolGradients[i] = new Gradient();
            colorKeys[i] = new GradientColorKey[fade[i-1].gradient.Length];
            alphaKeys[i] = new GradientAlphaKey[fade[i-1].gradient.Length];
            for(int j = 0; j < fade[i-1].gradient.Length; j++){
                colorKeys[i][j].color = fade[i-1].gradient[j];
                colorKeys[i][j].time = (float)j/(float)(fade[i-1].gradient.Length-1);
                // Debug.Log("Time for fade "+(i-1)+" color key "+j+" is "+colorKeys[i][j].time);
                alphaKeys[i][j].alpha = fade[i-1].gradient[j].a;
                alphaKeys[i][j].time = (float)j/(float)(fade[i-1].gradient.Length-1);
            }
    ...
    {    if(stateManager.IsPlaying()){
            ...
            for(int i = 1; i < myCoolGradients.Length; ++i){
                ...
                SpriteRenderer fadeSpriteRend = fade[i-1].fadeObj.GetComponent<SpriteRenderer>();
                fadeSpriteRend.color = fadeColor;
            }
